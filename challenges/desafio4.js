...
// Se quisermos analisar nossa coleção para encontrar títulos de filmes
// que têm uma só palavra no título,
...
// Crie uma pipeline que retorna documentos
// com o novo campo title_split, ela deve seguir as seguintes condições:
...
db.movies.aggregate([{ $project: { title_split: { $split: ["$title", " "] }, _id: 0 } }, { $match: { title_split: { $size: 1 } } }, { $sort: { title_split: 1 } }]);
